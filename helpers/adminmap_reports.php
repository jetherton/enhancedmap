...
class adminmap_reports_Core {
		...
		)
	{
		...
		$sql = 'SELECT incident.id, location_id, incident_title, incident_date, incident_active, location.latitude as lat, location.longitude as lon ';
		...
		)
	{
		...
		//start creating some SQL		
		$sql = 'SELECT incident.id, incident_mode, person.person_first as person_first, person.person_last as person_last, person.id as person_id, incident_description, incident.location_id, incident_title, incident_date, incident_active,  location.latitude as lat, location.longitude as lon ';
		$sql .= 'FROM  `'.$table_prefix.'incident` as `incident` ';
		$sql .= 'LEFT JOIN  `'.$table_prefix.'location` as `location` ON  `location`.id =  `incident`.`location_id` ';
